...
import {
  ...
  requireWebhookVerificationSecretVersion,
...
export let getTestGraphClientState = (version: number) =>
  `test-graph-client-state-v${version}`;

export let getTestGraphSubscriptionId = (version: number) =>
  `test-graph-subscription-v${version}`;

export let getTestGraphValidationToken = (activeVersion: number, retiringVersion: number) =>
  `test-graph-validation-v${activeVersion}-v${retiringVersion}-${randomUUID()}`;
...
let readPriorActiveAuthority = (value: unknown) => {
  ...
  if (
    ...
    typeof details.clientStateVersion !== 'number' ||
    !Number.isInteger(details.clientStateVersion) ||
    details.clientStateVersion <= 0
  ) {
    return null;
  }
  return {
    ...
    clientStateVersion: details.clientStateVersion
  };
};
...
export let createGraphWebhookRegistrationAuthority = (ctx: GraphRegistrationContext) => {
  let activeVersion = requireWebhookVerificationSecretVersion(
    ctx.input.capturedSecretVersions,
    graphActiveSecretRef.name
  );
  let retiringVersion = requireWebhookVerificationSecretVersion(
    ctx.input.capturedSecretVersions,
    graphRetiringSecretRef.name
  );
  let activeClientState = getTestGraphClientState(activeVersion);
  let subscriptionId = getTestGraphSubscriptionId(activeVersion);
  ...
  let retiringClientStateSourceVersion = priorActive?.clientStateVersion ?? activeVersion;
  let retiringClientState = getTestGraphClientState(retiringClientStateSourceVersion);
  ...
  return {
    ...
    capturedSecrets: {
      [graphActiveSecretRef.name]: {
        value: activeClientState,
        version: activeVersion
      },
      [graphRetiringSecretRef.name]: {
        value: retiringClientState,
        version: retiringVersion
      }
    }
  };
};
...
export let autoRegisterGraphWebhook = async (
  ...
  let activeVersion = authority.capturedSecrets[graphActiveSecretRef.name]!.version;
  let retiringVersion = authority.capturedSecrets[graphRetiringSecretRef.name]!.version;

  let validationToken = getTestGraphValidationToken(activeVersion, retiringVersion);
