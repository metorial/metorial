...
describe('Hub webhook verification contracts', () => {
  ...
  it('registers all Slack actions with the exact fixed secret and captured version authority', async () => {
    for (let [index, actionKey] of slackActionKeys.entries()) {
      ...
      expect(result).toEqual({
        ...
        capturedSecrets: {
          hmac_secret: { value: TEST_HMAC_SECRET, version }
        }
      });
    }
  });

  it('rejects missing or non-positive version authority for all Slack actions', async () => {
    for (let actionKey of slackActionKeys) {
      let action = provider.actions.find(candidate => candidate.key === actionKey);
      if (!action || action.type !== 'trigger' || !action.autoRegisterWebhook) {
        throw new Error(`Expected ${actionKey} to auto-register`);
      }
      let register = (capturedSecretVersions: Record<string, number>) =>
        action.autoRegisterWebhook!(
          new SlateContext(
            {},
            {
              webhookBaseUrl: 'https://callbacks.example.test/receiver-secret',
      ...
      await expect(register({})).rejects.toThrow(/secret-version authority/i);
      await expect(register({ hmac_secret: 0 })).rejects.toThrow(/secret-version authority/i);
      await expect(register({ hmac_secret: -1 })).rejects.toThrow(/secret-version authority/i);
    }
  });
  ...
  it('registers every non-Graph preset with registration-backed fixed test credentials', async () => {
    ...
    for (let [index, testCase] of nonGraphPresetCases.entries()) {
      let secretName =
      ...
      let response = await SlatesProviderProtoHandlerManager.handleInput(manager, {
        ...
        params: {
          ...
          webhookBaseUrl: 'https://callbacks.example.test/receiver-secret',
          capturedSecretVersions: { [secretName]: version }
        }
      });
      if (!response || !('result' in response)) {
      ...
      expect(response.result.capturedSecrets).toEqual({
        [secretName]: { value: credentialValue, version }
      });
  ...
  it('registers deterministic Graph active and retiring authority after an exact echo', async () => {
    ...
    expect(JSON.stringify(initial.registrationDetails)).not.toContain(
      'test-graph-client-state-v41'
    );

    let renewed = await invokeGraphRegistration(
      {
        graph_client_state: 51,
        graph_retiring_client_state: 52
      },
      initial.registrationDetails
    );
    expect(renewed.capturedSecrets).toEqual({
      graph_client_state: {
        value: 'test-graph-client-state-v51',
        version: 51
      },
      graph_retiring_client_state: {
        value: 'test-graph-client-state-v41',
    ...
    expect(renewed.registrationDetails).toMatchObject({
      subscriptionId: 'test-graph-subscription-v51',
      clientStateVersion: 51,
      retiringSubscriptionId: 'test-graph-subscription-v41',
      retiringClientStateSourceVersion: 41,
      subscriptions: [
        {
  ...
  it('rejects failed Graph validation echoes and missing exact version authority', async () => {
    let fetchMock = vi.fn();
    vi.stubGlobal('fetch', fetchMock);

    let invalidVersionCases: Record<string, number>[] = [
      {},
      { graph_client_state: 0, graph_retiring_client_state: 2 },
      { graph_client_state: 1, graph_retiring_client_state: -1 },
      { graph_client_state: 1.5, graph_retiring_client_state: 2 }
    ];
    for (let capturedSecretVersions of invalidVersionCases) {
      await expect(invokeGraphRegistration(capturedSecretVersions)).rejects.toThrow(
        /secret-version authority/i
      );
    }
    ...
    await expect(
      invokeGraphRegistration({
        graph_client_state: 61,
        graph_retiring_client_state: 62
      })
    ).rejects.toThrow(/status 200/i);

    fetchMock.mockResolvedValueOnce(new Response('wrong-echo', { status: 200 }));
    await expect(
      invokeGraphRegistration({
        graph_client_state: 63,
        graph_retiring_client_state: 64
      })
    ).rejects.toThrow(/exact validation token/i);
  });
  ...
  it('registers provider tokens and Graph authority without a preset-only self-probe', async () => {
    ...
    for (let [index, testCase] of providerCases.entries()) {
      let providerTokenVersion = index + 101;
      let graph = testCase.verifierId === 'graph.change_notification.provider.v1';
      let capturedSecretVersions: Record<string, number> = graph
        ? {
            provider_token: providerTokenVersion,
            graph_client_state: providerTokenVersion + 100,
            graph_retiring_client_state: providerTokenVersion + 200
          }
        : { provider_token: providerTokenVersion };
      let response = await SlatesProviderProtoHandlerManager.handleInput(manager, {
        jsonrpc: '2.0',
      ...
      expect(response.result.capturedSecrets.provider_token).toEqual({
        value: TEST_PROVIDER_TOKEN,
        version: providerTokenVersion
      });
      if (graph) {
        expect(response.result.capturedSecrets).toEqual({
          provider_token: {
          ...
          graph_client_state: {
            value: getTestGraphClientState(providerTokenVersion + 100),
            version: providerTokenVersion + 100
          },
          graph_retiring_client_state: {
            value: getTestGraphClientState(providerTokenVersion + 100),
            version: providerTokenVersion + 200
          }
        });
      } else {
        expect(response.result.capturedSecrets).toEqual({
      ...
      if (graph) {
        expect(registrationMetadata).not.toContain(
          getTestGraphClientState(providerTokenVersion + 100)
        );
      }
    }
    expect(fetchMock).not.toHaveBeenCalled();
  ...
  it('registers each core action with its exact fixed credential and requested version', async () => {
    let manager = await createProviderHandler(provider, []).run();
    for (let message of [
      {
        jsonrpc: '2.0' as const,
    ...
    for (let testCase of cases) {
      let response = await SlatesProviderProtoHandlerManager.handleInput(manager, {
        ...
        params: {
          ...
          webhookBaseUrl: 'https://callbacks.example.test/receiver-secret',
          capturedSecretVersions: { [testCase.secretName]: testCase.version }
        }
      });
      if (!response || !('result' in response)) {
  ...
  it('rejects missing or non-positive secret-version authority in every core handler', async () => {
    let cases = [
      { actionId: 'verify_static_header', secretName: 'static_token' },
      { actionId: 'verify_static_query', secretName: 'static_token' },
      { actionId: 'verify_static_json', secretName: 'static_token' },
      { actionId: 'verify_raw_hmac', secretName: 'hmac_secret' },
      { actionId: 'verify_ed25519', secretName: 'ed25519_public_key' }
    ];

    for (let testCase of cases) {
      let action = provider.actions.find(candidate => candidate.key === testCase.actionId);
      if (!action || action.type !== 'trigger' || !action.autoRegisterWebhook) {
        throw new Error(`Expected ${testCase.actionId} to auto-register`);
      }
      ...
      await expect(register({})).rejects.toThrow(/secret-version authority/i);
      await expect(register({ [testCase.secretName]: 0 })).rejects.toThrow(
        /secret-version authority/i
      );
      await expect(register({ [testCase.secretName]: -1 })).rejects.toThrow(
        /secret-version authority/i
      );
    }
  });
