...
let GRAPH_TRIGGER_KEYS = new Set<WebhookTestTriggerKey>([
...
let parseGraphAuthorityVersion = (value: string | undefined) => {
  if (value === undefined || value.startsWith('--')) {
    throw new TypeError('Graph authority version value is required');
  }
  if (!/^[1-9]\d*$/.test(value)) {
...
export let parseSendWebhookArguments = (argv: readonly string[]): SendWebhookCliOptions => {
  ...
  let dryRun = false;
  let bootstrap = false;
  let graphAuthorityVersion: number | undefined;
  ...
  for (let index = 2; index < argv.length; index += 1) {
    ...
    if (
      ...
      argument !== '--dry-run' &&
      argument !== '--bootstrap' &&
      argument !== '--graph-authority-version' &&
      argument !== '--slack-case'
    ) {
    ...
    if (argument === '--invalid') {
      ...
      bootstrap = true;
    } else if (argument === '--graph-authority-version') {
      graphAuthorityVersion = parseGraphAuthorityVersion(argv[index + 1]);
      index += 1;
    } else {
  ...
  if (graphAuthorityVersion !== undefined && !GRAPH_TRIGGER_KEYS.has(triggerKey)) {
    throw new TypeError('Graph authority version is only valid for Graph trigger keys');
  }
  ...
  if (bootstrap && graphAuthorityVersion !== undefined) {
    throw new TypeError('Graph authority version is not used in bootstrap mode');
  }
  ...
  return {
    ...
    ...(graphAuthorityVersion === undefined ? {} : { graphAuthorityVersion })
  };
};
...
let requestOptions = (
  ...
  ...(options.graphAuthorityVersion === undefined
    ? {}
    : { graphAuthorityVersion: options.graphAuthorityVersion })
});
...
let buildCliRequest = (
  ...
  let commonOptions = requestOptions(options, dependencies);
...
let writeDryRun = (
  ...
  if (GRAPH_TRIGGER_KEYS.has(options.triggerKey) && !options.bootstrap) {
    write(`Graph authority version: ${options.graphAuthorityVersion ?? 1}`);
  }
};
...
export let SEND_WEBHOOK_USAGE =
  'Usage: node dist/send-webhook/index.js <trigger-key> <callback-url> [--invalid] [--bootstrap] [--dry-run] [--graph-authority-version <positive-int>] [--slack-case <case>]';
