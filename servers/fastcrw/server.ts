...
/**
 * fastCRW MCP Server
 * Provides web scraping and crawling capabilities through the fastCRW API.
 *
 * fastCRW is a Firecrawl-compatible web data engine in a single binary; it can be
 * self-hosted or used via the managed cloud at https://fastcrw.com. Because it is
 * Firecrawl-compatible, this server mirrors the Firecrawl provider and simply points
 * at the fastCRW base URL.
 *
 * Config:
 *   - token: API key (Bearer). Read from CRW_API_KEY in the host environment.
 *     Optional for self-hosted servers that run without auth.
 *   - apiUrl: optional base URL override for self-hosting
 *     (e.g. http://localhost:3000). Defaults to the managed cloud.
 */

metorial.createServer<{ token: string; apiUrl?: string }>(
  {
    name: 'fastcrw-server',
    version: '1.0.0'
  },
  async (server, config) => {
    // Default to the managed cloud; allow overriding the base URL for self-hosting.
    const API_BASE_URL = (config.apiUrl ?? 'https://fastcrw.com/api').replace(/\/+$/, '');

    // ============================================================================
    // Type Definitions
    // ============================================================================

    interface FastcrwError {
      error?: string;
      success?: boolean;
      error_code?: string;
      code?: string;
    ...
    /**
     * Make an authenticated request to the fastCRW API
     */
    async function fastcrwRequest<T>(
      endpoint: string,
      options: RequestInit = {}
    ): Promise<T> {
      ...
      const response = await fetch(url, {
        ...options,
        headers: {
          // Bearer auth; self-hosted servers may run without auth, in which case
          // the (possibly empty) token is simply ignored by the server.
      ...
      if (!response.ok) {
        const errorData = (await response.json().catch(() => ({}))) as FastcrwError;
        throw new Error(
          errorData.error || `fastCRW API error: ${response.status} ${response.statusText}`
        );
      }
    ...
    server.registerTool(
      'scrape_url',
      {
        ...
        inputSchema: {
          ...
          storeInCache: z.boolean().optional().describe('Store page in fastCRW cache'),
          zeroDataRetention: z.boolean().optional().describe('Enable zero data retention mode')
        }
      },
      async params => {
        try {
          const response = await fastcrwRequest<ScrapeResponse>('/v1/scrape', {
            method: 'POST',
    ...
    server.registerTool(
      ...
      async params => {
        try {
          const response = await fastcrwRequest<CrawlResponse>('/v1/crawl', {
            method: 'POST',
            body: JSON.stringify(params)
          });
    ...
    server.registerTool(
      ...
      async ({ crawlId, limit, next }) => {
        try {
          let endpoint = `/v1/crawl/${encodeURIComponent(crawlId)}`;
          const params = new URLSearchParams();
          if (limit) params.append('limit', String(limit));
          if (next) params.append('next', next);
          if (params.toString()) endpoint += `?${params.toString()}`;

          const response = await fastcrwRequest<CrawlStatusResponse>(endpoint, {
            method: 'GET'
          });
    ...
    server.registerTool(
      ...
      async ({ crawlId }) => {
        try {
          const response = await fastcrwRequest(`/v1/crawl/${encodeURIComponent(crawlId)}`, {
            method: 'DELETE'
          });
    ...
    server.registerTool(
      'map_url',
      {
        title: 'Map URL',
        description:
          'Map a website to quickly discover all of its URLs without scraping their content. Returns a list of links found on the site.',
        inputSchema: {
          url: z.string().url().describe('The base URL to map'),
          search: z
            .string()
            .optional()
            .describe('Filter discovered links by this search term'),
          limit: z.number().optional().describe('Maximum number of links to return'),
          includeSubdomains: z
            .boolean()
            .optional()
            .describe('Include links from subdomains'),
          ignoreSitemap: z
            .boolean()
            .optional()
            .describe('Ignore the sitemap and discover links from the page')
        }
      },
      async params => {
        try {
          const response = await fastcrwRequest<MapResponse>('/v1/map', {
            method: 'POST',
            body: JSON.stringify(params)
          });

          if (!response.success) {
            throw new Error(response.error || 'Failed to map URL');
          }
    ...
    server.registerTool(
      'search',
      {
        title: 'Search',
        description:
          'Search the web and optionally scrape the results. Returns a list of matching results with title, URL, and description (and markdown when scraping is requested). Available on the managed cloud.',
        inputSchema: {
          ...
          sources: z
            ...
            .optional()
            .describe('Result sources to search (web, images)'),
          scrapeOptions: scrapeOptionsSchema
            .optional()
            .describe('When provided, scrape each result with these options')
        }
      },
      async params => {
        try {
          const response = await fastcrwRequest<SearchResponse>('/v1/search', {
            method: 'POST',
            body: JSON.stringify(params)
          });
          ...
          const text = results
            .map((result, idx) => {
              const lines = [`## ${idx + 1}. ${result.title || result.url || 'Untitled'}`];
    ...
    server.registerResource(
      'scraped-page',
      new ResourceTemplate('fastcrw://scraped/{url}', { list: undefined }),
      {
        title: 'Scraped Page',
    ...
    server.registerResource(
      'crawl-job',
      new ResourceTemplate('fastcrw://crawl/{crawlId}', { list: undefined }),
      {
        title: 'Crawl Job',
      ...
      async (uri, { crawlId }) => {
        try {
          const response = await fastcrwRequest<CrawlStatusResponse>(
            `/v1/crawl/${encodeURIComponent(crawlId as string)}`,
            { method: 'GET' }
          );
    ...
    server.registerResource(
      'crawl-pages',
      new ResourceTemplate('fastcrw://crawl/{crawlId}/pages', { list: undefined }),
      {
    ...
    server.registerResource(
      'crawl-page',
      new ResourceTemplate('fastcrw://crawl/{crawlId}/page/{pageIndex}', {
        list: undefined
      }),
      {
