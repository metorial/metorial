...
describe('Task 3 webhook RPC contracts', () => {
  ...
  it('returns captured secrets as plain name-to-value records owned by the caller', () => {
    expect(
      slatesWebhookBootstrapCaptureOutput.safeParse({
        status: 'accepted',
        capturedSecrets: { signing_secret: 'secret-value' },
        response: {
          status: 200,
          headers: [],
          body: { present: false }
    ...
    expect(
      slatesWebhookBootstrapCaptureOutput.safeParse({
        status: 'accepted',
        capturedSecrets: { signing_secret: { value: 'secret-value', version: 1 } },
        response: {
          status: 200,
          headers: [],
          body: { present: false }
