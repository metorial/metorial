...
describe('webhook wire v1 fixed conformance fixtures', () => {
  it.each(
    SLATE_WEBHOOK_WIRE_CONFORMANCE_FIXTURES_V1
  )('keeps $name bytes and request hash stable', fixture => {
    expect(
...
describe('webhook verification contracts', () => {
  ...
  it('accepts exact bound secret refs and rejects invalid source shapes', () => {
    let validRefs = [
      ...
      },
      {
        ...
        callbackSecretKey: 'signing_secret',
        encoding: 'utf8'
      }
    ];
    ...
    let invalidRefs = [
      ...
      { ...validRefs[3], credentialKey: 'x'.repeat(129) },
      { ...validRefs[0], credentialKey: 'cross_source' },
      { ...validRefs[1], registrationKey: 'cross_source' },
      { ...validRefs[3], callbackSecretKey: 'cross_source' },
      { ...validRefs[4], registrationKey: 'cross_source' },
      { ...validRefs[5], credentialKey: 'cross_source' },
      {
        source: 'config',
        name: 'legacy_config_secret',
  ...
  it('enforces unique published names and verifier access by explicit named refs', () => {
    ...
    expect(
      slatesWebhookVerification.safeParse({
        ...valid,
        allowedSecretRefs: [declaredSecret, { ...declaredSecret, credentialKey: 'other' }]
      }).success
    ).toBe(false);
    let methodAwareRefs = [
      {
      ...
      {
        ...declaredSecret,
        authMethods: ['token', 'system_user']
      }
    ];
    expect(
      slatesWebhookVerification.safeParse({
        ...valid,
        allowedSecretRefs: methodAwareRefs
      }).success
    ).toBe(true);
    expect(
      slatesWebhookVerification.safeParse({
        ...
        allowedSecretRefs: [
          methodAwareRefs[0],
          { ...methodAwareRefs[1], authMethods: ['oauth', 'token'] }
        ]
      }).success
    ).toBe(false);
    expect(
...
describe('ingress matrix', () => {
  it.each([
    ...
    'provider',
    'path_secret_only'
  ] as const)('accepts an exact receiver-route %s declaration', mechanism => {
