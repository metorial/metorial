...
export let slatesWebhookSecretRef = z.discriminatedUnion('source', [
  z.strictObject({
    source: z.literal('auth_config'),
    name: identifier,
    credentialKey: secretKey,
    authMethods: z.array(identifier).min(1).optional(),
    encoding: z.enum(SLATE_WEBHOOK_SECRET_ENCODINGS)
  }),
  z.strictObject({
    source: z.literal('oauth_credentials'),
    name: identifier,
    credentialKey: secretKey,
    authMethods: z.array(identifier).min(1).optional(),
    encoding: z.enum(SLATE_WEBHOOK_SECRET_ENCODINGS)
  ...
  z.strictObject({
    source: z.literal('callback_secret'),
    name: identifier,
    callbackSecretKey: secretKey,
    encoding: z.enum(SLATE_WEBHOOK_SECRET_ENCODINGS)
  }),
  z.strictObject({
...
let validateSecretRefs = (
  ...
  let secretNames = new Map<string, z.infer<typeof slatesWebhookSecretRef>[]>();
  secretRefs.forEach((secretRef, index) => {
    if (
      'authMethods' in secretRef &&
      secretRef.authMethods &&
      new Set(secretRef.authMethods).size !== secretRef.authMethods.length
    ) {
      context.addIssue({
        code: 'custom',
        path: ['allowedSecretRefs', index, 'authMethods'],
        message: `Webhook secret auth methods must be unique: ${secretRef.name}`
      });
    }

    let existing = secretNames.get(secretRef.name) ?? [];
    let currentMethods = 'authMethods' in secretRef ? secretRef.authMethods : undefined;
    let disjointMethodAlternatives =
      currentMethods !== undefined &&
      existing.every(existingRef => {
        let existingMethods =
          'authMethods' in existingRef ? existingRef.authMethods : undefined;
        return (
          existingMethods !== undefined &&
          existingMethods.every(method => !currentMethods.includes(method))
        );
      });
    if (existing.length > 0 && !disjointMethodAlternatives) {
      context.addIssue({
        code: 'custom',
    ...
    secretNames.set(secretRef.name, [...existing, secretRef]);
  });
