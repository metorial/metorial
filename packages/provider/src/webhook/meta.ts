...
export let metaWebhookHttp = (d: {
  oauthAuthMethods: string[];
  authConfigAuthMethods: string[];
}) =>
  ({
    ...
    ingress: {
      ...
      verification: {
        ...
        allowedSecretRefs: [
          ...
          ...(d.oauthAuthMethods.length > 0
            ? [
                {
                  source: 'oauth_credentials' as const,
                  name: 'meta_app_secret',
                  credentialKey: 'clientSecret',
                  authMethods: d.oauthAuthMethods,
                  encoding: 'utf8' as const
                }
              ]
            : []),
          ...(d.authConfigAuthMethods.length > 0
            ? [
                {
                  source: 'auth_config' as const,
                  name: 'meta_app_secret',
                  credentialKey: 'webhookAppSecret',
                  authMethods: d.authConfigAuthMethods,
                  encoding: 'utf8' as const
                }
              ]
            : [])
