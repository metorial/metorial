...
export let registerWordWebhook = async (
  ...
  if (prior?.retiringSubscriptionId && prior.retiringSubscriptionId !== prior.subscriptionId) {
    await client.deleteSubscription(prior.retiringSubscriptionId);
  }
  ...
  let activeVersion = ctx.input.capturedSecretVersions.graph_client_state;
  let retiringVersion = ctx.input.capturedSecretVersions.graph_retiring_client_state;
  if (!activeVersion || !retiringVersion) {
    throw new Error('Graph client-state secret-version authority is missing');
  }
  let capturedSecrets = {
    graph_client_state: { value: clientState, version: activeVersion },
