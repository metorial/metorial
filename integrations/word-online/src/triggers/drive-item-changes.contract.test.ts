...
describe('Word Online drive_item_changes lifecycle contract', () => {
  ...
  it('persists immutable subscription authority separately from initial delta state', async () => {
    ...
    expect(result).toMatchObject({
      ...
      capturedSecrets: {
        graph_client_state: expect.stringMatching(/^[A-Za-z0-9_-]{43}$/),
        graph_retiring_client_state: expect.stringMatching(/^[A-Za-z0-9_-]{43}$/)
      }
    });
  ...
  it('creates replacement authority, retires prior active state, and removes older overlap', async () => {
    ...
    expect(result.capturedSecrets).toEqual({
      graph_client_state: expect.stringMatching(/^[A-Za-z0-9_-]{43}$/),
      graph_retiring_client_state: 'active-client-state'
    });
    expect(JSON.stringify(result.capturedSecrets)).not.toContain('version');
  });
