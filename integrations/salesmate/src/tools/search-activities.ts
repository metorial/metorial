...
let searchRuleSchema = z.object({
  fieldName: z
    .string()
    .describe('Field name to filter on (e.g., "title", "type", "owner", "isCompleted")'),
  condition: z
    .string()
  ...
  .input(
    z.object({
      fields: z
        .array(z.string())
        .describe(
          'List of field names to return (e.g., ["title", "type", "dueDate", "isCompleted"])'
        ),
  ...
  .output(
    z.object({
      activities: z
        ...
        .describe('Array of matching activity records'),
