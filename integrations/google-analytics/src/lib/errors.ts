...
export let googleAnalyticsServiceError = (message: string) =>
  new ServiceError(badRequestError({ message }));

export let googleAnalyticsApiError = (error: unknown, operation = 'request') => {
  ...
  let serviceError = googleAnalyticsServiceError(
    `Google Analytics API ${operation} failed: ${statusLabel}${extractGoogleMessage(error)}`
  );

  serviceError.data.reason = 'google_analytics_api_error';
...
export let googleAnalyticsOAuthError = (operation: string, error: unknown) => {
  if (error instanceof ServiceError) {
    return error;
  }

  let serviceError = googleAnalyticsServiceError(
    `Google Analytics OAuth ${operation} failed: ${extractGoogleMessage(error)}`
  );
  let status = getErrorStatus(error);
  serviceError.data.reason = 'google_analytics_oauth_error';
  serviceError.data.upstreamStatus = status;
