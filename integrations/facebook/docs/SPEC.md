...
Webhook setup requires providing the generated **Callback URL** (HTTPS with valid TLS/SSL) and **Verify Token** in the Facebook app. The callback lifecycle owns the verify token and requires an exact match before echoing Facebook's challenge synchronously. Ordinary POST notifications remain asynchronous and must carry a valid HMAC-SHA256 signature made with the App Secret from the linked OAuth credentials.
