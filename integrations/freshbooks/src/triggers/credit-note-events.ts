...
export let creditNoteEvents = SlateTrigger.create(spec, {
  name: 'Credit Note Events',
  key: 'credit_note_events',
  description: 'Triggered when credit notes are created, updated, or deleted in FreshBooks.'
})
  .input(
    z.object({
      eventName: z.string(),
  ...
  .output(
    z.object({
      creditNoteId: z.string().describe('ID of the affected credit note'),
      accountId: z.string().describe('FreshBooks account ID'),
      eventName: z.string().describe('Full event name (e.g. credit_note.create)'),
  ...
  .webhook({
    autoRegisterWebhook: async ctx => {
      ...
      let callback = await client.createWebhook('credit_note', ctx.input.webhookBaseUrl);
    ...
    handleEvent: async ctx => {
      ...
      return {
        type: `credit_note.${action}`,
        id: `${ctx.input.eventName}-${ctx.input.objectId}-${Date.now()}`,
        output: {
          creditNoteId: ctx.input.objectId,
