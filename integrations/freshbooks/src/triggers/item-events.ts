...
export let itemEvents = SlateTrigger.create(spec, {
  name: 'Item Events',
  key: 'item_events',
  description: 'Triggered when billable items are created, updated, or deleted in FreshBooks.'
})
  .input(
    z.object({
      eventName: z.string(),
  ...
  .output(
    z.object({
      ...
      eventName: z.string().describe('Full event name (e.g. item.create)'),
  ...
  .webhook({
    autoRegisterWebhook: async ctx => {
      ...
      let callback = await client.createWebhook('item', ctx.input.webhookBaseUrl);
