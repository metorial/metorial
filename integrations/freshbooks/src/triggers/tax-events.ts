...
export let taxEvents = SlateTrigger.create(spec, {
  name: 'Tax Events',
  key: 'tax_events',
  description:
    'Triggered when tax configurations are created, updated, or deleted in FreshBooks.'
})
  .input(
    z.object({
      eventName: z.string(),
  ...
  .output(
    z.object({
      taxId: z.string().describe('ID of the affected tax'),
      accountId: z.string().describe('FreshBooks account ID'),
      eventName: z.string().describe('Full event name (e.g. tax.create)'),
  ...
  .webhook({
    autoRegisterWebhook: async ctx => {
      ...
      let callback = await client.createWebhook('tax', ctx.input.webhookBaseUrl);
