...
export let timeEntryEvents = SlateTrigger.create(spec, {
  ...
  description: 'Triggered when time entries are created, updated, or deleted in FreshBooks.'
})
  .input(
    z.object({
      eventName: z.string(),
  ...
  .output(
    z.object({
      ...
      eventName: z.string().describe('Full event name (e.g. time_entry.create)'),
  ...
  .webhook({
    autoRegisterWebhook: async ctx => {
      ...
      let callback = await client.createWebhook('time_entry', ctx.input.webhookBaseUrl);
    ...
    handleEvent: async ctx => {
      ...
      return {
        type: `time_entry.${action}`,
        id: `${ctx.input.eventName}-${ctx.input.objectId}-${Date.now()}`,
        output: {
          timeEntryId: ctx.input.objectId,
