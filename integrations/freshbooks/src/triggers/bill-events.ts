...
export let billEvents = SlateTrigger.create(spec, {
  name: 'Bill Events',
  key: 'bill_events',
  description: 'Triggered when bills are created, updated, or deleted in FreshBooks.'
})
  .input(
    z.object({
      eventName: z.string(),
  ...
  .output(
    z.object({
      billId: z.string().describe('ID of the affected bill'),
      accountId: z.string().describe('FreshBooks account ID'),
      eventName: z.string().describe('Full event name (e.g. bill.create)'),
  ...
  .webhook({
    autoRegisterWebhook: async ctx => {
      ...
      let callback = await client.createWebhook('bill', ctx.input.webhookBaseUrl);
