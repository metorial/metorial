...
export let invoiceEvents = SlateTrigger.create(spec, {
  ...
  key: 'invoice_events',
  description:
    'Triggered when invoices are created, updated, deleted, or sent by email in FreshBooks.'
})
  .input(
    z.object({
      eventName: z.string(),
  ...
  .output(
    z.object({
      invoiceId: z.string().describe('ID of the affected invoice'),
      accountId: z.string().describe('FreshBooks account ID'),
      eventName: z.string().describe('Full event name (e.g. invoice.create)'),
  ...
  .webhook({
    autoRegisterWebhook: async ctx => {
      ...
      let callback = await client.createWebhook('invoice', ctx.input.webhookBaseUrl);
