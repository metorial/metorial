...
let addressSchema = z
  ...
  .describe(
    'Address of the invoice recipient. Provide either contactId or inline address fields.'
  );
...
let taxConditionsSchema = z
  .object({
    taxType: z
      ...
      .describe('Tax type for the invoice')
  })
  .describe('Tax conditions for the invoice');
...
export let createInvoice = SlateTool.create(spec, {
  name: 'Create Invoice',
  key: 'create_invoice',
  description: `Creates a new invoice in Lexoffice. Supports specifying a recipient by contact ID or inline address, adding line items with pricing, tax conditions, payment terms, and shipping conditions. The invoice can optionally be finalized immediately or linked to a preceding sales voucher.`,
  ...
  .input(
    z.object({
      ...
      taxConditions: taxConditionsSchema.describe('Tax conditions for the invoice'),
      paymentConditions: paymentConditionsSchema
        .optional()
        .describe('Payment conditions for the invoice'),
      shippingConditions: shippingConditionsSchema
        .optional()
        .describe('Shipping or service date conditions'),
      title: z.string().optional().describe('Custom title for the invoice'),
      introduction: z
        .string()
        .optional()
      ...
      voucherDate: z
        .string()
        .optional()
        .describe('Invoice date in ISO 8601 format (e.g. 2024-01-15). Defaults to today.'),
      finalize: z
        .boolean()
        .optional()
        .describe(
          'If true, the invoice is finalized immediately and cannot be edited further'
        ),
      precedingSalesVoucherId: z
        .string()
        .optional()
        .describe('ID of a preceding sales voucher (e.g. a quotation) to link to this invoice')
    })
  )
  .output(
    z.object({
      id: z.string().describe('Unique ID of the created invoice'),
      resourceUri: z.string().describe('URI to access the invoice resource'),
      createdDate: z.string().describe('Timestamp when the invoice was created'),
      updatedDate: z.string().describe('Timestamp when the invoice was last updated'),
      version: z.number().describe('Version number of the invoice for optimistic locking')
    })
  )
  .handleInvocation(async ctx => {
    ...
    if (ctx.input.totalPrice) invoiceData.totalPrice = ctx.input.totalPrice;
    if (ctx.input.paymentConditions)
      invoiceData.paymentConditions = ctx.input.paymentConditions;
    if (ctx.input.shippingConditions)
      invoiceData.shippingConditions = ctx.input.shippingConditions;
    if (ctx.input.title) invoiceData.title = ctx.input.title;
    if (ctx.input.introduction) invoiceData.introduction = ctx.input.introduction;
    if (ctx.input.remark) invoiceData.remark = ctx.input.remark;
    if (ctx.input.voucherDate) invoiceData.voucherDate = ctx.input.voucherDate;
    ...
    return {
      ...
      message: `Created invoice **${result.id}**${ctx.input.finalize ? ' (finalized)' : ' (draft)'}`
    };
  })
  .build();
