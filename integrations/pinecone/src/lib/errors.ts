...
let statusLabel = (response?: ErrorResponse) =>
  response?.status !== undefined
    ? `HTTP ${response.status}${response.statusText ? ` ${response.statusText}` : ''}: `
    : '';

export let pineconeServiceError = (message: string) =>
  new ServiceError(badRequestError({ message }));

export let pineconeApiError = (error: unknown, operation = 'request') => {
  if (error instanceof ServiceError) {
    return error;
  }

  let response = errorResponse(error);
  let serviceError = pineconeServiceError(
    `Pinecone API ${operation} failed: ${statusLabel(response)}${extractMessage(error)}`
  );
  serviceError.data.reason = 'pinecone_api_error';
