...
let extractDynamoDbMessage = (error: unknown) => {
...
let extractDynamoDbStatus = (error: unknown) => {
...
let extractDynamoDbCode = (error: unknown) => {
  if (!isRecord(error)) return undefined;
...
export let dynamoDbServiceError = (message: string) =>
  new ServiceError(badRequestError({ message }));

export let dynamoDbApiError = (error: unknown, operation = 'request') => {
  if (error instanceof ServiceError) {
    return error;
  }

  let response = isRecord(error) ? (error.response as ErrorResponse | undefined) : undefined;
  let status = extractDynamoDbStatus(error);
  let code = extractDynamoDbCode(error);
  let statusLabel =
    status !== undefined
  ...
  let serviceError = dynamoDbServiceError(
    `Amazon DynamoDB API ${operation} failed: ${statusLabel}${codeLabel}${extractDynamoDbMessage(error)}`
  );

  serviceError.data.reason = 'aws_dynamodb_api_error';
