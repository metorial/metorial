...
describe('Telegram singleton webhook contract', () => {
  ...
  it('compares an active or retiring token exactly and rejects malformed input', () => {
    let request = (value?: string) =>
      new Request('https://example.com/telegram', {
        headers: value ? { 'x-telegram-bot-api-secret-token': value } : {}
      });
    expect(
      verifySecretToken(request('active-token'), ['active-token', 'retiring-token'])
    ).toBe(true);
    expect(
      verifySecretToken(request('retiring-token'), ['active-token', 'retiring-token'])
    ).toBe(true);
    expect(verifySecretToken(request('wrong-token'), ['active-token', 'retiring-token'])).toBe(
      false
    );
  ...
  it('programs one receiver URL with the exact union while preserving its bound token', async () => {
    let result = await registerTelegramWebhook(
      {
        ...
        input: {
          ...
          capturedSecretVersions: { telegram_secret_token: 4 }
        }
      },
      ['callback_query']
    );
    ...
    expect(result.capturedSecrets).toEqual({
      telegram_secret_token: { value: 'stable-token', version: 4 }
    });
  });
