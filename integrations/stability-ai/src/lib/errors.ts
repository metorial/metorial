...
let collectMessages = (value: unknown, messages: string[]) => {
  ...
  for (let key of ['message', 'detail', 'error', 'title', 'name']) {
...
let getErrorStatusText = (error: unknown) => getErrorResponse(error)?.statusText;
...
export let stabilityServiceError = (message: string) =>
  new ServiceError(badRequestError({ message }));

export let stabilityApiError = (error: unknown, operation = 'request') => {
  ...
  let serviceError = stabilityServiceError(
    `Stability AI API ${operation} failed: ${statusLabel}${extractMessage(error)}`
  );

  serviceError.data.reason = 'stability_ai_api_error';
