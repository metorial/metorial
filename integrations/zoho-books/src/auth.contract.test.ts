...
let createBooksClient = (authOutput: Record<string, unknown>) =>
  createClient({ auth: authOutput, config: { organizationId: 'organization-id' } } as any);

describe('Zoho Books auth and config contract', () => {
  beforeEach(() => {
  ...
  it('constructs the client from canonical auth output', () => {
    expect(() => createBooksClient(canonicalAuth)).not.toThrow();
  ...
  it.each([
    ...
    let value = { ...canonicalAuth } as Record<string, unknown>;
    delete value[field];
    expect(() => createBooksClient(value)).toThrow(message);
  });
});
