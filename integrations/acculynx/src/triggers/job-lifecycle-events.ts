...
let JOB_LIFECYCLE_TOPICS = ['job_created', 'job_updated'];

export let jobLifecycleEventsTrigger = SlateTrigger.create(spec, {
  name: 'Job Lifecycle Events',
  key: 'job_lifecycle_events',
  description: 'Triggered when a job is created or updated in AccuLynx.'
})
  .input(
    z.object({
  ...
  .webhook({
    autoRegisterWebhook: async ctx => {
      ...
      let subscription = await client.createSubscription({
        ...
        topicNames: JOB_LIFECYCLE_TOPICS
      });
    ...
    handleRequest: async ctx => {
      ...
      let inputs = events.map((event: any) => ({
        topicName: event.topicName ?? event.topic ?? event.type ?? 'job_updated',
    ...
    handleEvent: async ctx => {
      ...
      let type = topicName === 'job_created' ? 'job.created' : 'job.updated';
