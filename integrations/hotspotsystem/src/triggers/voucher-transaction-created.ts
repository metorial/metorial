...
export let voucherTransactionCreated = SlateTrigger.create(spec, {
  name: 'Voucher Transaction Created',
  key: 'voucher_transaction_created',
  description:
    'Triggered when a new voucher-based access transaction is recorded. Configure the webhook in the HotspotSystem Control Center under Tools & Settings > Webhooks with the event type "transaction.voucher.create".'
})
  .input(
    z.object({
  ...
  .webhook({
    ...
    handleEvent: async ctx => {
      ...
      return {
        type: 'transaction.voucher.created',
        id: ctx.input.transactionId || `voucher-tx-${Date.now()}`,
