...
let extractElevenLabsMessage = (error: unknown) => {
...
export let elevenLabsServiceError = (message: string) =>
  new ServiceError(badRequestError({ message }));

export let elevenLabsApiError = (error: unknown, operation = 'request') => {
  if (error instanceof ServiceError) {
    return error;
  }

  let response = isRecord(error) ? (error.response as ErrorResponse | undefined) : undefined;
  let serviceError = elevenLabsServiceError(
    `ElevenLabs API ${operation} failed: ${statusLabelFor(response)}${extractElevenLabsMessage(error)}`
  );
  serviceError.data.reason = 'elevenlabs_api_error';
