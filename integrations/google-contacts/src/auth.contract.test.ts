  ...
  vi.doMock('slates', async () => {
    ...
    return {
      ...
      createAxios: vi.fn((config?: { baseURL?: string }) => {
        ...
        if (config?.baseURL === 'https://people.googleapis.com/v1/') {
          return {
            get: profileGet
          };
        }
...
describe('google-contacts auth contract', () => {
  it('builds the expected OAuth authorization URL', async () => {
    ...
    let result = await client.getAuthorizationUrl({
      ...
      scopes: [googleContactsScopes.contactsReadonly, googleContactsScopes.userInfoProfile]
    });
    ...
    expect(url.searchParams.get('scope')).toBe(
      `${googleContactsScopes.contactsReadonly} ${googleContactsScopes.userInfoProfile}`
    );
  });
  ...
  it('maps callback and refresh token responses into the stored auth shape', async () => {
    ...
    oauthPost.mockResolvedValueOnce({
      data: {
        ...
        expires_in: 3600,
        scope: `${googleContactsScopes.contacts} ${googleContactsScopes.contactsOtherReadonly}`
      }
    });
  ...
  it('maps the People API profile payload into the Slate profile shape', async () => {
    let client = await loadProviderClient();

    profileGet.mockResolvedValueOnce({
      data: {
        resourceName: 'people/me',
        names: [{ displayName: 'Contacts Test User' }],
        emailAddresses: [{ value: 'contacts-test@example.com' }],
        photos: [{ url: 'https://example.com/photo.png' }]
      }
    });
    ...
    let result = await client.getAuthProfile({
      ...
      input: {},
      scopes: [googleContactsScopes.userInfoProfile]
    });
