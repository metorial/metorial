...
let extractBrowserbaseMessage = (error: unknown) => {
...
export let browserbaseServiceError = (message: string) =>
  new ServiceError(badRequestError({ message }));

export let browserbaseApiError = (error: unknown, operation = 'request') => {
  if (error instanceof ServiceError) {
    return error;
  }

  let response = isRecord(error) ? (error.response as ErrorResponse | undefined) : undefined;
  let serviceError = browserbaseServiceError(
    `Browserbase API ${operation} failed: ${statusLabelFor(response)}${extractBrowserbaseMessage(error)}`
  );
  serviceError.data.reason = 'browserbase_api_error';
