    ...
    verification: {
      ...
      allowedSecretRefs: [
        ...
        },
        {
          ...
          credentialKey: 'auth.clientId',
          encoding: 'utf8'
        },
        {
          source: 'platform',
          name: 'paypal_client_secret',
          credentialKey: 'auth.clientSecret',
          encoding: 'utf8'
        },
        {
          source: 'platform',
          name: 'paypal_environment',
          credentialKey: 'auth.environment',
          encoding: 'utf8'
        }
      ],
      rules: [
...
export let paypalRegistrationResult = (
  webhookId: string,
  capturedSecretVersions: Readonly<Record<string, number>>
) => {
  let version = capturedSecretVersions.paypal_webhook_id;
  if (!version) throw new Error('PayPal webhook secret-version authority is missing');
  return {
    registrationDetails: { webhookId },
    capturedSecrets: { paypal_webhook_id: { value: webhookId, version } }
  };
};
