...
export let collectionEvents = SlateTrigger.create(spec, {
  ...
  description:
    'Triggers when new marketplace events occur for a collection — including sales, transfers, listings, cancellations, and redemptions.'
})
  .input(
    z.object({
      eventType: z
  ...
  .polling({
    ...
    pollEvents: async ctx => {
      ...
      let collectionSlug = (ctx.state as any)?.collectionSlug;
      if (!collectionSlug) {
      ...
      let data = await client.getEventsByCollection(collectionSlug, {
        after: afterParam,
        limit: 50
      });
      ...
      let inputs = events.map((e: any) => {
        ...
        return {
          ...
          collectionSlug: e.collection_slug ?? e.nft?.collection ?? collectionSlug,
          maker: e.maker ?? null,
          taker: e.taker ?? null,
