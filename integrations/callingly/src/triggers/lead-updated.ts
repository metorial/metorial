...
export let leadUpdated = SlateTrigger.create(spec, {
  name: 'Lead Updated',
  key: 'lead_updated',
  description:
    'Fires when a lead is updated. Can optionally be filtered to only trigger when a specific lead field has changed and matches a given value.'
})
  .input(
    z.object({
  ...
  .output(
    z.object({
      ...
      updatedAt: z.string().optional().describe('When the lead was last updated')
    })
  )
  .webhook({
    ...
    handleEvent: async ctx => {
      ...
      return {
        type: 'lead.updated',
        id: `${ctx.input.leadId}-${raw.updated_at ?? Date.now()}`,
        output: {
