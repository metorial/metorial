...
import { Client, ZOHO_CRM_API_ORIGINS } from './lib/client';
...
describe('Zoho CRM auth and config contract', () => {
  beforeEach(() => {
  ...
  it.each(
    Object.entries(ZOHO_CRM_API_ORIGINS)
  )('allowlists the %s CRM API origin', (region, apiDomain) => {
    expect(() => createClient({ ...canonicalAuth, region, apiDomain })).not.toThrow();
    expect(httpCalls.configs.at(-1)?.baseURL).toBe(`${apiDomain}/crm/v8`);
  });

  it('rejects an API origin outside the CRM allowlist', () => {
    expect(() =>
      createClient({ ...canonicalAuth, apiDomain: 'https://crm.example.com' })
