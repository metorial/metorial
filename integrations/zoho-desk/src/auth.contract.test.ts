...
let createDeskClient = (authOutput: Record<string, unknown>) =>
  createClient({ auth: authOutput, config: { orgId: 'organization-id' } } as any);

describe('Zoho Desk auth and config contract', () => {
  beforeEach(() => {
  ...
  it('emits a closed top-level config object without regional routing', () => {
    ...
    expect(Object.keys(configSchema.properties ?? {})).toEqual(['orgId']);
    expect(configSchema.properties).not.toHaveProperty('region');
  });

  it('strips the removed region field from legacy config', () => {
    expect(config.configSchema.parse({ orgId: 'organization-id', region: 'eu' })).toEqual({
      orgId: 'organization-id'
    });
  });

  it.each(
    Object.entries(ZOHO_DESK_API_ORIGINS)
  )('constructs the %s client from the validated provider-returned API domain', (_region, apiDomain) => {
    expect(() => createDeskClient({ ...canonicalAuth, apiDomain })).not.toThrow();
    expect(httpCalls.configs.at(-1)?.baseURL).toBe(`${apiDomain}/api/v1`);
  });

  it('rejects an unrecognized provider-returned API domain', () => {
    expect(() =>
      createDeskClient({ ...canonicalAuth, apiDomain: 'https://desk.example.com' })
  ...
  it.each([
    ...
    let value = { ...canonicalAuth } as Record<string, unknown>;
    delete value[field];
    expect(() => createDeskClient(value)).toThrow(message);
  });
});
