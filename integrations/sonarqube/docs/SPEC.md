...
- SonarQube Server reachability and system status through `api/system/ping` and `api/system/status`

Intentionally omitted capabilities include entitlement-gated enterprise or commercial workflows outside the focused analysis surface, local IDE and SonarLint operations, and deferred webhook or administrative monitoring APIs such as system health, system information, and logs. Project lifecycle management, token/user/group administration, branch deletion, quality gate administration, Compute Engine administration, measure history, broad component browsing, issue changelog reads, and rule search also remain out of scope.
...
All tool input schemas are top-level `z.object` schemas to remain MCP/OpenAI tool bridge compatible. Conditional requirements are described in field descriptions and enforced at runtime with `ServiceError`. `search_my_sonarqube_projects.q` performs project-name or project-key search through components search. `list_branches` returns all analyzed branches and locally applies its optional `branchTypes` filter (`ALL`, `LONG`, or `SHORT`) after fetching `/api/project_branches/list`; it preserves the Cloud `mergeBranch` target for short-lived branches. Pull request analyses stay in `list_pull_requests`.

The SonarQube MCP Server v1.23 parameter standardization is reflected in the affected tool contracts:

- `search_my_sonarqube_projects` uses `pageIndex` instead of `page`.
- `search_metrics`, `search_security_hotspots`, and `search_sonar_issues_in_projects` use `pageIndex` and `pageSize` instead of `p` and `ps`.
- `search_sonar_issues_in_projects` uses `projectKeys` instead of `projects`.
- `run_advanced_code_analysis` uses the required `branch` parameter instead of `branchName`.
- `search_dependency_risks` keeps `pageIndex` and `pageSize` as the canonical numeric pagination inputs, matching the upstream number schema.

`search_sonar_issues_in_projects` also exposes `files`, `issueKey`, `severities`, `issueStatuses`, and `impactSoftwareQualities`. `run_advanced_code_analysis` mirrors the official workspace-backed input contract (`projectKey`, `branch`, `filePath`, `fileScope`), reads `filePath` from the current local workspace, sends the content to the A3S API, and returns `issues`, `patchResult`, and `analysisErrors`. Unlike the official server, which hides the tool without a configured workspace mount, this package keeps it discoverable and returns `ServiceError` when the requested workspace file is unavailable. `search_dependency_risks` mirrors the official SCA response with `issuesReleases` and `paging`. `ping_system` has an exact empty-object input schema, sends an anonymous SonarQube Server request, and exposes the plain-text `pong` response as `{ response: "pong" }`. `search_files_by_coverage` mirrors the official coverage search (component tree sorted ascending by the `coverage` metric, files without coverage dropped, `maxCoverage` filter inclusive) and `get_file_coverage_details` computes the official line/branch coverage summary from `api/sources/lines` records.

Outputs expose normalized fields for agent workflows. `search_duplicated_files` discovers file component keys with duplication metrics before `get_duplications` retrieves detailed duplication blocks. Source-code content is returned as a downloadable text file, not in inline output fields. No tool returns base64 blobs or full source text in structured output.

## Verification

Package tests cover config/base URL validation, query parameter serialization, component-search project discovery, project qualifier filtering, issue severity routing, branch/pull-request mutual exclusion, hotspot project/key/new-code parameters, duplicated-file discovery, branch type filtering and merge-target mapping, pagination caps, Cloud organization checks, project-key fallback, quality gate identifier validation, Server-only status and ping validation, anonymous ping request routing, structured ping output, source attachment metadata, authentication response validation, advanced-analysis and dependency-risk endpoint routing, issue-search schema compatibility, and Sonar error normalization.

Schema regression coverage uses `describeMcpCompatibleToolSchemas('SonarQube tool input schemas', provider.actions)`.

Private live E2E coverage lives in `tests/integrations/sonarqube/tools.e2e.ts`. It includes the Server-only ping and can discover a readable project from the profile, or use optional fixtures for `projectKey`, `projectSearchQuery`, `metricKeys`, `branch`, `pullRequest`, `issueKey`, hotspot keys, rule keys, mutable finding keys, source component keys, dependency-risk enablement, and advanced-analysis file content.
